...
class Events(commands.Cog):
    ...
    async def on_app_command_error(
        self, interaction: discord.Interaction, error: Union[app_commands.AppCommandError, Exception]
    ) -> Any:
        ...
        if isinstance(error, TO_IGNORE):
            ...
            await interaction.response.send_message(
                "This command is on cooldown... " f"try again in {error.retry_after:.2f} seconds.", ephemeral=True
            )
        else:
