...
class FunCommands(commands.Cog):
    ...
    async def randomimageapi(self, interaction: discord.Interaction[DiscordBot], url: str, *endpoint: str) -> None:
        try:
            ...
            return await interaction.response.send_message(
                "The API returned an error or didn't return JSON...", ephemeral=True
            )
    ...
    async def coinflip(self, interaction: discord.Interaction[DiscordBot]):
        ...
        await interaction.response.send_message(
            f"**{interaction.user.name}** flipped a coin and got **{random.choice(coinsides)}**!"
        )
    ...
    async def reverse(self, interaction: discord.Interaction[DiscordBot], text: str):
        ...
        await interaction.response.send_message(f"🔁 {t_rev}", allowed_mentions=discord.AllowedMentions.none())

    @app_commands.command()
    @app_commands.describe(
        obytes="The amount of numbers to generate. Defaults to 18",
        dm="Whether to DM the password or not. Defaults to False",
    )
    ...
    @app_commands.command()
    @app_commands.describe(user="The user to give a beer to.", reason="The reason why you are giving the beer.")
    async def beer(
        self,
        ...
        *,
        ...
        if user.bot:
            return await interaction.response.send_message(
                f"I would love to give beer to the bot **{interaction.user.name}**, but I don't think it will respond to you :/"
            )

        beer_offer = f"**{user.name}**, you got a 🍺 offer from **{interaction.user.name}**" + (
        ...
        try:
            ...
            await interaction.edit_original_response(
                content=f"**{user.name}** and **{interaction.user.name}** are enjoying a lovely beer together 🍻"
            )
    ...
    async def noticeme(self, interaction: discord.Interaction[DiscordBot]):
        ...
        await interaction.response.send_message(
            file=discord.File(BytesIO(r.response), filename="noticeme.gif")  # type: ignore
        )
    ...
    async def dice(self, interaction: discord.Interaction[DiscordBot]):
        ...
        results = "\n".join(
            [f"**{self.bot.user.display_name}:** 🎲 {bot_dice}", f"**{interaction.user.display_name}** 🎲 {player_dice}"]
        )
    ...
    async def roulette(
        self, interaction: discord.Interaction[DiscordBot], picked_colour: Literal["blue", "red", "green", "yellow"]
    ):
        """Colours roulette"""
    ...
    async def amiadmin(self, interaction: discord.Interaction[DiscordBot]):
        ...
        #   -- AlexFlipnote
            ...
            return await interaction.response.send_message(
                f"Well kinda **{interaction.user.name}**.. " "you still own the source code"
            )
